...
{
    public class sixteencards_CardDeck : MonoBehaviour
{
    public static sixteencards_CardDeck instance;
    Card[] deckCards;
    Sprite playNormal;
    Sprite playSelected;
   ...
   public Sprite GetChipImage(int value)
{
    ...
    if (chipImages == null || chipImages.Length == 0)
    {
        Debug.LogError("chipImages array is empty or not assigned!");
        return null; // Return null or a default value as needed
    }

    // Iterate through the array to find the matching sprite
    ...
    // Fallback to the first element if no match is found
    return chipImages[0];
}
