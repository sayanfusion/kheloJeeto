...
{
    public sealed class BallisticTrajectoryEquation
    {
        ...
        public static int SolveQuartic(double c0, double c1, double c2, double c3, double c4, out double s0, out double s1, out double s2, out double s3)
        {
            ...
            if (IsZero(r))
            {
                ...
                num = BallisticTrajectoryEquation.SolveCubic(coeffs[0], coeffs[1], coeffs[2], coeffs[3], out s0, out s1, out s2);
            }
            else
            {
                ...
                num = BallisticTrajectoryEquation.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s0, out s1);

                coeffs[2] = z + u;
                coeffs[1] = q < 0 ? v : -v;
                coeffs[0] = 1;

                if (num == 0) num += BallisticTrajectoryEquation.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s0, out s1);
                if (num == 1) num += BallisticTrajectoryEquation.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s1, out s2);
                if (num == 2) num += BallisticTrajectoryEquation.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s2, out s3);
            }
        ...
        public static float BallisticRange(float speed, float gravity, float initial_height)
        {
            ...
            Debug.Assert(speed > 0 && gravity > 0 && initial_height >= 0, "BallisticTrajectoryEquation.BallisticRange called with invalid data");
        ...
        public static int SolveBallisticArc(Vector3 proj_pos, float proj_speed, Vector3 target, float gravity, out Vector3 s0, out Vector3 s1)
        {
            ...
            // Handling these cases is up to your project's coding standards
            Debug.Assert(proj_pos != target && proj_speed > 0 && gravity > 0, "BallisticTrajectoryEquation.SolveBallisticArc called with invalid data");
        ...
        public static bool SolveBallisticArcLateral(Vector3 proj_pos, float lateral_speed, Vector3 target_pos, float max_height, out Vector3 fire_velocity, out float gravity)
        {
            ...
            // Handling these cases is up to your project's coding standards
            Debug.Assert(proj_pos != target_pos && lateral_speed > 0 && max_height > proj_pos.y, "BallisticTrajectoryEquation.SolveBallisticArcLateral called with invalid data");
        ...
        public static bool SolveBallisticArcLateral(Vector3 proj_pos, float lateral_speed, Vector3 target, Vector3 target_velocity, float max_height_offset, out Vector3 fire_velocity, out float gravity, out Vector3 impact_point)
        {
            ...
            // Handling these cases is up to your project's coding standards
            Debug.Assert(proj_pos != target && lateral_speed > 0, "BallisticTrajectoryEquation.SolveBallisticArcLateral called with invalid data");
            ...
            double t0, t1;
            int n = BallisticTrajectoryEquation.SolveQuadric(c0, c1, c2, out t0, out t1);
