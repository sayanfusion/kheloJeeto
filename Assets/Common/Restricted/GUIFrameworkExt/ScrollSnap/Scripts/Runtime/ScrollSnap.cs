...
{
    public interface IScrollSnapEvent
    {
        void OnTraverse(ScrollSnap.ETraverse a_Traverse);
    }
    ...
    public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public List<IScrollSnapEvent> ScrollSnapEvent = new List<IScrollSnapEvent>();
        ...
        private void SetTraverseState()
        {
            if (CurrentPanel == 0)
            {
                ScrollSnapEvent.ForEach(x => x.OnTraverse(ETraverse.BackwardComplete));
            }
            else if (CurrentPanel == Panels.Length - 1)
            {
                ScrollSnapEvent.ForEach(x => x.OnTraverse(ETraverse.ForwardComplete));
            }
            else
            {
                ScrollSnapEvent.ForEach(x => x.OnTraverse(ETraverse.InBetween));
            }
        }
